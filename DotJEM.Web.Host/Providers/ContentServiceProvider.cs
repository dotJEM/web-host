...
namespace DotJEM.Web.Host.Providers
{
    public class ContentServiceProvider : ServiceProvider<IContentService>
    {
        ...
        public ContentServiceProvider(IStorageIndex index, IStorageContext context, IStorageIndexManager manager, IPipeline pipeline, IJsonMergeVisitor merger)
            : base(name => new ContentService(index, context.Area(name), manager, pipeline, merger))
        {
