...
namespace DotJEM.Web.Host.Providers.Index
{
    ...
    public class MultiFieldQueryParser : QueryParser, IQueryParser
    {
        ...
        public MultiFieldQueryParser(IJsonIndex index, string query)
            : this(index, query, index.Schemas.AllFields().ToArray())
        {
        }

        public MultiFieldQueryParser(IJsonIndex index, string query, params string[] fields)
        ...
        protected override Query GetRangeQuery(string field, string part1, string part2, bool startInclusive, bool endInclusive)
        {
            ...
            {
                var query = PrepareBuilderFor(field)
                    .BuildRangeQuery(new CallContext(() => base.GetRangeQuery(field, part1, part2, startInclusive, endInclusive)), part1, part2, inclusive);
                return query;
            }
            return GetBooleanQuery(fields
